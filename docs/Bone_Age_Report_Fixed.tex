...
\subsection{Classification Model - Developmental Stages}
...
\subsection{Interpretation of Errors and Difficult Samples}
...
\begin{enumerate}
    \item \textbf{Xception's preprocessing:} Using architecture-specific preprocessing fixed issues from generic rescaling
    \item \textbf{Transfer learning:} ImageNet pre-training provided excellent feature extraction
    \item \textbf{Simple design:} Just 10 dense units helped avoid overfitting on medical data
    \item \textbf{Training strategy:} Training all layers from epoch 1 outperformed gradual unfreezing
\end{enumerate}

\textbf{Challenges Encountered:}
\begin{enumerate}
    \item \textbf{GPU memory constraints:} Limited to batch size of 4 on RTX 3060 12GB
    \item \textbf{Preprocessing experimentation:} Multiple failed attempts before finding correct approach
    \item \textbf{Class imbalance:} Fewer samples at age extremes affected performance
\end{enumerate}
